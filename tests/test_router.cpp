...
protected:
  void SetUp() override { router = std::make_unique<Router>(); }
...
TEST_F(RouterTest, MatchesWildcardPath) {
  ...
  // Should also match the base path without trailing slash
  auto match3 = router->matchRoute("/api/users");
...
TEST_F(RouterTest, RoundRobinLoadBalancing) {
  ...
  route.backends = {"http://localhost:3001", "http://localhost:3002",
                    "http://localhost:3003"};
